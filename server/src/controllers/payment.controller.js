...
export const initiatePayment = async (req, res, next) => {
    try {
        ...
        if (booking.user.toString() !== req.user._id.toString()) { const err = new Error("Not authorized for this booking"); err.statusCode = 403; throw err; }
        if (booking.status !== "Pending" || booking.paymentStatus !== "unpaid") { const err = new Error("Booking is not awaiting payment"); err.statusCode = 409; throw err; }
...
export const verifyPayment = async (req, res, next) => {
    try {
        ...
        if (!orderId || !paymentId || !signature) { const err = new Error("Missing payment verification fields"); err.statusCode = 400; throw err; }
        if (!paymentEngine.verifyPaymentSignature({ orderId, paymentId, signature })) {
            const err = new Error("Payment signature verification failed"); err.statusCode = 400; throw err;
        }

        const booking = await Booking.findOne({ _id: bookingId, paymentOrderId: orderId });
        if (!booking) { const err = new Error("Booking not found for this order"); err.statusCode = 404; throw err; }
